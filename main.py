...
def send_sms(msg, templateId):
    try:
        ...
        response = requests.get("http://mysms.onlinebusinessbazaar.in/api/mt/SendSMS?user=wellwin&password=sms123&senderid=VTRAKK&channel=Trans&DCS=0&flashsms=0&number=&text="+ msg +"&route=06&DLTTemplateId="+templateId+"&PEID=1201159282315113937", timeout=60)
        print(response.text)
    except Exception as e:
        print("sms error")
